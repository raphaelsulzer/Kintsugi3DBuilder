...
void main()
{
    ...
    for (int k = 0; k < CAMERA_POSE_COUNT; k++)
    {
        ...
        if (imgColor.a > 0 && dot(normal, getViewVector(k, position)) > 0)
        {
            ...
            // Combines all LIGHTS_PER_VIEW lights' intensity as one -- an approximation that's fine here since
            // this pass is only used for initial coarse clustering, not precise reflectance recovery.
            vec3 rgb = imgColor.rgb / getCombinedLightIntensity(k, position);
